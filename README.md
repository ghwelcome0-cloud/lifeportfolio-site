...
- 신규 생성(`/report-loading?sid=...`): 개인/단체 모두 네 축 모델을 포함해 기존 저장 절차를 따른다. 새 모듈 실패 시 저장 전 중단한다. 기존 저장 리포트는 열기만 해서는 이관하지 않는다.
- 개인 `/report?sid=...` 재생성: 본인 원응답과 현재 저장본으로 네 축만 갱신한다. ETag 조건부 PUT으로 다른 탭/수동 검수 변경을 덮지 않고, 응답 유실 후 중복 쓰기를 하지 않는다. 동일 결과의 반복 갱신은 쓰지 않는다. 수동 검수본·응답 부재·계정 변경·읽기/쓰기 실패는 보존 우선이다. 점수·고유코드·다른 페이지·프로그램·인덱스에는 변경을 만들지 않는다.
- 재생성 저장 확인은 RTDB의 키 순서 정렬·null/빈 배열/빈 객체 제거를 반영한 값 비교를 사용한다. 단순 JSON 문자열 비교로 저장 성공을 실패로 표시하던 결함을 수정했다. 실제 내용 변경은 계속 거절하며, 빈 판단 목록의 성공 표시와 동일 결과 재시도의 무쓰기를 보장한다. `LP_AXIS_RTDB_EMULATOR=1 node scripts/test-axis-refresh.cjs`는 127.0.0.1:9105의 demo namespace만 사용해 실제 로컬 REST 왕복·응답 유실·반복 무쓰기를 검사한다.
- 단체 기존 저장본의 재생성은 서버의 진로 3필드 보정 계약을 유지한다. 이번 public-only 배포에서 단체 저장본 전체/네 축을 클라이언트로 덮지 않는다. 단체의 신규 생성에는 네 축 모델이 포함된다.
- `응답 근거`는 새 모델의 판단 출처·남길 결과·미해석 근거를 보여 준다. 미지원 문형은 기존 해석을 유지하며 새롭게 해석했다고 표시하지 않는다. 한국어 관계 판단 범위는 승인 후보와 같고 영어 결과는 기존 문구를 보존한다.
- 검증: `npm run test:axes`(순수 판단, 실제 재생성 함수의 합성 전송, 실제 리포트 DOM/14지면·양 인쇄판·PC/모바일). 비-VII 12지면의 텍스트와 전체 프로그램 출력 비교를 포함한다. 전체 npm/동일 head CI 및 Chromium/WebKit 검증 후 기존 Firebase `hosting:public` 워크플로만 사용한다. 실기기·실고객 로그인/결제/메일·고객 DB 조사는 하지 않는다.
- 브라우저 비교는 실제 `LP_I18N.onReady`와 언어/사전 일치를 확인한 뒤 수행한다. 번역 사전 1.5초 지연 조건에서도 13쪽을 포함한 비-VII 동일성을 검사한다. 모바일 목차는 전환 애니메이션 종료와 클릭 지점의 실제 노출을 확인한 뒤 네이티브 클릭한다. 비교 제외/재시도/제품 문구 변경으로 검사를 우회하지 않는다.
- 최종 배포 상태는 이 브랜치 PR의 release 기록을 정본으로 한다. 아래 로컬 후보/미배포 문구는 승인 전 이력이다.

## 네 축만의 근거 관계 엔진 — 승인 전 로컬 검토 이력

- 범위 재설정: 네 축 우선. 기존 PR333 신규 방식이 기존보다 고유성·내용 품질이 높다는 판단은 사용자 피드백에 따라 철회했다. 운영 PR333을 임의로 롤백하거나 기존 저장본을 전환하지 않는다.
- `LPResponseEvidence.projectAxes({questions, answers, baseline, lang})`를 추가했다. 승인된 기존 네 축 화면 문구를 입력받고, 출처가 있는 관계만 core/detail/action/reflection 사본에 반영한다. 원 리포트·점수·지문·키워드·네 축 밖 페이지·프로그램에는 쓰지 않는다. 기존 compile/생성/저장/재생성 경로는 바꾸지 않았다.
- 현재 지원 관계: 선택 기준×판단 근거, 주변이 전한 표현 방식×관계에서 지킬 것, 직접입력의 대상/내용/수단×확인 가능한 이해, 실행 방식×리듬/지속 조건. 직접입력의 열린 역할 추출은 제한된 한국어 명시 문형과 시범/실행 수단에 한정된다. 영어와 미지원 문맥은 기존 표현을 유지하며 미해석/검토 필요를 기록한다. 일반 NLP나 80억 명 검증 완료가 아니다.
- 사실(facts), 해석 가설(decisions), 출력별 근거(fieldSources), 미해석(unresolved/notInterpreted)을 분리한다. 각 제안은 확인 동작(operation)과 남길 결과(artifact), 다시 쓸 목적(reuse)을 갖는다. 성경은 최상위 규범 기준이며 알고리즘의 경험적 타당도를 대신하지 않는다. 벤치마크는 출처 구분·작동 조건·개인 내 확인으로, 수학적 설계는 정보 보존·순서/이름 불변·중요 근거의 통제 비교로 제한해 적용한다.
- 실행: `node scripts/test-axis-projection.cjs --artifact`. `dist/device-guide-review/axis-projection-review.html`은 네 축의 같은 기존 굵은 문장을 공유하던 합성 응답 쌍을 비교한다. 실제 본인 원응답을 대신하거나 실제 PDF 재생성 완료로 표시하지 않는다.
- 남은 단계: 비교 승인, 미해석·복합 근거의 적용 정책 및 실제 VII 화면 연결 검증. 함수는 아직 서비스에서 호출하지 않으므로 운영 URI/DB/API/배포는 변하지 않는다. 전체 서비스 전환 및 저장본 재생성 적용은 완료하지 않았다. 비용 절감을 위해 이 후보는 외부 에이전트/새 CI/운영 배포 없이 로컬에서 검토한다.

## 네 축 최종 교정 후보 (2026-09-22)

- 추가 검토 반례 `못합니다/cannot/unable`의 긍정 권고를 차단하고, 하기 어려운 행동과 스스로 피하는 행동을 구분해 확인한다. `돕고 싶다` 등 바람도 실제 선택으로 단정하지 않는다.
- 열린 우선순위는 두 문법(`성과보다 약속을…`, `약속을 성과보다…`)을 구분하고 정도 부사(`훨씬 더`)를 기준에 넣지 않는다. 해석하지 못한 비교문은 기존 특정 단어 폴백 대신 확인 질문으로 보낸다. 사람 유형이나 기준 목록을 추가한 것이 아니다.
- 20개 부모 질문의 실행 문장 첫머리를 `…에 관해 적어 주신 내용을 살펴보면`으로 바꾸어 `활동로/기준로/방식로` 조사 오류를 제거했다. 기존 핵심 문장 golden과 나열형 거절 규칙은 유지한다.
- 배포 전 후보이며 동일 head 전체 회귀·CI·검토를 통과해야 한다. 최종 배포 상태·manifest·run은 [PR333](https://github.com/ghwelcome0-cloud/lifeportfolio-site/pull/333)의 release 기록을 정본으로 한다. 아래 미배포/차단 기록은 각 후보 당시 이력이다. Firebase public-only, 원응답·과거 저장본·승인 V8·가격·계약 보존 범위는 변경하지 않는다.

## 근거 사용 관문 보완 · PR333 배포 전 검증 (2026-09-22)

- c338d8f 검토에서 `안 합니다/안 해요`, 전언, 후행 타인 소유 표현을 잘못 긍정 해석한 결함을 재현했고 해당 후보 배포를 보류했다. 수정본은 방법 선택 전에 부정·주체·가정·불확실성·질문을 검사하고, 원문 좌표 `qualifierSpans`와 사용 가능 여부 `admission`을 남긴다.
- 확인 질문도 일괄 문구가 아니다. 피하려는 행동과 조건 / 타인의 생각과 내 입장 / 가정과 실제 선택을 구분하며 남길 결과도 각각 다르게 정한다. 원칙·자연스러운 문장·현재 버전 경계는 유지한다.
- 회귀: 검토 반례4개와 KO/EN 변형, 긍정 대조, 20부모 문맥 공통 관문, 보고서→프로그램의 확인 필요 상태 전파. 정확성의 범위는 합성 반례이며 일반 자연어 이해나 모든 서비스 전환 완료를 뜻하지 않는다.
- 기존 툴바 전체화면 30초 타임아웃은 기준 변경 없이 단독 재검사에서 통과했다. 원인 확정/최초 실패 삭제 없이 최종 후보 전체 검사를 다시 수행한다.
- 배포 상태: 수정 후보 검증 중. 현재 제품 운영은 PR331이며, PR333의 최종 배포는 동일 head 검사와 reviewed artifact 검증 후 기존 Firebase public-only workflow로 수행한다. 실제 배포 완료 여부는 아래 release 기록이 아니라 PR333 최종 receipt를 확인한다.

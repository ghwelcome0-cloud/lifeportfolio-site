...
## VII 네 축 읽기 개선 (2026-09-21, Firebase public 배포 후보)
...
- 배포 상태: 아직 후보. 기존 PR CI·artifact/provenance·canonical main rebuild 후 `hosting:public`만 배포한다. Functions/admin/RTDB·Firestore rules 및 Cloudflare 배포는 하지 않는다. 실제 고객 로그인·결제·메일 수신·실기기 E2E는 미실시다. 제작규칙서 전면 최신화와 그 안의 성경 원칙 반영은 이번 작업 후 별도로 진행한다.

...
- 승인 댓글 형식: `APPROVE_B2B_MAIL_SMOKE <실행할-main-SHA> <FIXTURE_SHA> <RECIPIENT_SHA>`. 두 해시는 `scripts/send-b2b-mail-smoke.mjs`에 고정. 기존 홈페이지 승인 댓글 재사용 금지. 새 절차도 기존 PR 필수 승인·검사를 통과한 뒤에만 실행.
...
- POST는 메시지당 1회, timeout 후 자동 재시도 없음. 고정 idempotency key 사용; 공급자의 보존기간 이후에는 중복 방지가 보장되지 않으므로 새 수동 실행 전 이전 영수증/수신 상태 확인 필수. GitHub run 재실행은 차단.
...
- 로컬 검증: `node scripts/send-b2b-mail-smoke.mjs --dry-run`, `node scripts/test-b2b-mail-smoke.mjs`. 시험 주소 환경변수가 있을 때 추가 mock 6시나리오(접수/422/503/timeout/ID누락/GET거부) 실행. 실제 발송 0건.

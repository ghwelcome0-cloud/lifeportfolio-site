...
## 독립 검토 후 보완 (2026-09-18, 운영 미반영)

- I장 영문 참고 직업 라벨의 초기화 순서 오류를 고쳤다. 책 전체에 영문이 하나 있는지만 보지 않고 모든 참고 라벨의 언어를 개별 검사한다.
- 명시적 재생성 확인 범위를 참고 예시뿐 아니라 진로·교육·확장 방향의 제거/교체까지 넓혔다. 수동본 보호는 유지한다.
- 지문 검증은 리포트 최초 생성·재생성 경로에 한정된다. program-loading/program의 자료 로딩까지 검증된 것은 아니다.
- 106개 Node 회귀와 18개 브라우저 회귀를 기존 검사를 모두 유지한 `npm test`에 추가했다. `scripts/test-all.mjs`는 불변 계약 보호 대상이므로 원본을 유지했다. 브라우저 검사는 원본 HTML의 Firebase import를 합성 SDK로 대체한 검사이며 실제 Firebase 인증·고객 자료 검증이 아니다. 리포트 참고 라벨 및 보존 회귀 18개 재실행 통과.
- 출시 보류 항목(분야 역할/참고자료 선택/1회권리/실제 E2E)은 그대로이며 이번 보완을 출시 승인으로 해석하지 않는다.

## 재개 후 로더·표시 보호 교정 (2026-09-18, 운영 미반영)

- 필수 JSON 4개를 SHA-256 지문으로 검증한다. 빈 JSON·구형/변조 자료를 거부하고 `semantic-resources-2026-09-18-v2` 계약이 없는 엔진을 다시 로드한다. 재시도·병렬 로드 시 미완료 요청을 기다린 후 상태를 해제한다. 규칙 파일 수정 시 지문과 회귀 검사를 함께 갱신해야 한다.
- 최초생성과 재생성의 기본 엔진에도 버전 검증·12초 제한을 적용했다. 기존 수동 교정본은 자동 재생성으로 덮어쓰지 않는다. 이전 참고 예시가 삭제되는 재생성은 제거 목록을 확인받으며 취소 시 저장 호출을 하지 않는다.
- 웹/책 표시에서 생성된 참고 예시의 앞 4개 절단을 제거했다. KO/EN 라벨을 비순위 탐색 예시로 교정하고 생성 근거를 책 payload 및 인쇄용 근거 페이지까지 연결했다. 이는 엔진의 첫 항목 선택·6개 절단·분야 치환 자체의 해결을 의미하지 않는다.
- 실행 검증: `node scripts/test-report-semantic-integrity.cjs` 106개 PASS. `scripts/test-report-browser-integrity.cjs` 실제 HTML+Chromium 18개 PASS: KO/EN·375/1280폭 책 렌더, 최초/재생성의 빈 자료·구형 규칙·404·구형 엔진·스크립트 오류에서 저장 0, 예시 제거 취소·수동본 보호. 모든 네트워크는 로컬 응답 또는 차단, Auth/RTDB는 합성이며 운영 접속·실기기·실제 PDF 수신 증거가 아니다.
- 브라우저 검사는 설치된 puppeteer를 사용한다. 선택 환경변수 `LP_TEST_DEPENDENCIES`(package.json이 있는 의존성 루트), `LP_BROWSER_PATH`(Chromium 실행파일). 기존 검사기를 느슨하게 만들지 않고 별도 회귀를 추가했다.
- 남은 출시 조건: 분야의 숨은 역할 순서와 참고자료 의미 교정, 전체 정책/가독성 및 동일-head 독립 검토, 단체 권리 서버 확정, 실제 승인된 E2E·메일·배포. 승인 V8/미디어·고객 자료를 변경하지 않았다.

## 의미 정합성 교정 후보 — 2026-09-18 / 운영 미반영

- 별도 브랜치 `audit/report-semantic-integrity`, 기준 main `211504f`. 고객 응답·리포트·구매권 조회/변경 0, 기존 저장 결과 자동 재생성 없음. Firebase와 승인 V8/미디어 유지.
- 실제 공개 소스 10개가 로컬 기준과 동일함을 확인한 후 Node 합성 160회, Chromium 엔진 40회로 생성경로 차이·이름 오인·클릭 순서·6분야 누락·공백에 따른 훈련 변경을 재현. 이는 통계적 타당도/직무적합성/실고객 E2E 인증이 아님.
- 좁은 교정: 메타 Q1/Q2를 진로 신호에서 제외, Q3~Q5를 강점 단어로 오인하지 않음. 원문 저장은 바꾸지 않고 계산 사본만 NFC/trim/비순위 배열 정규화. 조건부 직접 입력을 리커트로 계산하지 않음.
- 최초 생성·명시적 재생성은 `ReportEngine.loadReportResources`로 같은 필수 규칙/엔진을 로드. 누락·불완전 생성 시 저장하지 않음. 기존 리포트를 자동 교체하지 않음.
- `career-engine.js`의 빠진 Q75 6선택지는 기존 V4 어휘와 동기화. 기존 13영역×5참고분류 자료를 삭제하지 않음. 성향을 특정 직업 자격이나 측정 역량으로 표현하지 않음.
- 검토 대상 신규 규칙: Q77 명시 활동→기존 참고 자료 분류 연결표. 참고 직업·훈련은 Q75/Q77 교차 근거와 분야 치환 내역을 보존하고 무관한 전체응답 해시로 교체하지 않음. 근거가 없는 직접 입력은 임의 직업으로 분류하지 않음. 진로·교육 본문을 단순 활동 문장 몇 개로 대체하는 축소 실험은 폐기했고 이 브랜치에 포함하지 않음.
- 로컬: `node scripts/test-report-semantic-integrity.cjs` 92개 합성 검사 통과. 문법 검사 통과. 전체 기존 품질 검사·실제 페이지/PDF 렌더·영문 가독성·사용자 타당성 검토는 아직 완료하지 않음.
- 남은 핵심: 정규화된 분야를 다시 숨은 우선순위로 해석하지 않도록 대칭/근거 기반 역할 배정, 가중치·참고자료 선택의 독립 검토, 모든 표시 경로의 근거 노출, 기존 품질 규약과 충돌 검토. 이 후보를 완성된 맞춤 진단이나 출시 가능 상태로 표현하지 않음.

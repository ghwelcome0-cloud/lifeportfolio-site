...
| R10 | q90Entitlements writer (approved migration) or provider verifier exists, else the normal first-purchaser path is documented as **closed** (it is, today: `payments/{uid}` alone → `ENTITLEMENT_UNVERIFIED`). Interface + synthetic contract tests: `entitlement-writer-contract.js` / `test/entitlement-writer-contract.test.js`; verifier `verified`-only-if conditions = W regression N03/N07–N12/N16 (3871081). Writer **self-validates** every record (`checkEntitlementRecord`) — Admin SDK bypasses rules, so rules are not a control here (3871771) | owner + legal gate (impl), TL (interface) | separate order |

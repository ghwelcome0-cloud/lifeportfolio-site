...
SUBS = [
    # S1 hook (0~6.02) — 슬라이드가 메인. 자막 생략(중복 방지)
    # S2 study (6.02~10.62) — 슬라이드가 메인. 자막 생략
    # B-roll (10.62~13.20) — 슬라이드 없음 → 핵심 자막 必
    (10.62, 13.20, "목적이 또렷한 사람은\\N{G}어떻게 달라졌을까{/G}"),
    # S3 numbers (13.20~20.02) — 슬라이드 카드(금액)가 메인. 자막 생략
    # S4 cta (20.02~38.53) — 슬라이드 태그/메인/CTA카드가 하단 안전구역까지 차지.
    #   → #1과 동일하게 S4 구간 자막 전면 생략(중복·겹침 방지).
]

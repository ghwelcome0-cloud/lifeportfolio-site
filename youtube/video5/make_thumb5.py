# -*- coding: utf-8 -*-
"""5편 썸네일: 안개 B-roll 배경 + navy+gold 텍스트. 1280x720.
주제: '불편함이라는 신호' — 방향을 반대편에서 찾기. 후크 카피."""
...
W,H=1280,720
bg=Image.open("thumb_bg5.png").convert("RGB").resize((W,H), Image.LANCZOS)

# 좌측 가독성을 위해 좌->우 navy 그라데이션 오버레이
ov=Image.new("RGB",(W,H),NAVY_DK)
mask=Image.new("L",(W,H),0); md=ImageDraw.Draw(mask)
for x in range(W):
    a=int(195*max(0,(1-x/(W*0.82))))  # 왼쪽 진하게
...
# 메인 카피 (3줄)
main_f=F(EB,104)
shadow_text((70,180),"무엇이",main_f,WHITE)
glow_text((70,300),"불편한가?",main_f,GOLD_LT,glow=GOLD,gr=14,ga=175)

# gold 디바이더
d.rectangle([74,452,74+380,458],fill=GOLD)

# 하단 보조 카피 (2줄)
sub_f=F(R,42)
shadow_text((74,486),"막막할 땐, 질문을 반대로",sub_f,WHITE,sa=160,sr=5,off=3)
sub_f2=F(R,36)
shadow_text((74,556),"불편함은 잡음이 아니라 '신호'다",sub_f2,GOLD_LT,sa=150,sr=5,off=3)
...
bg.save("thumbnail_5.png")
print("thumbnail saved -> thumbnail_5.png", bg.size)

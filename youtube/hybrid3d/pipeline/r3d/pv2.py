...
for f in range(1, F + 1):
    ...
    tgt.location = (0.0, -0.55 + 0.20 * e, 0.15 + 1.30 * ease(max(0.0, (u - 0.40) / 0.60)))
...
S.render.filepath = '/home/user/lf/r3d/_pv2/g_'
t = time.time(); bpy.ops.render.render(animation=True); el = time.time() - t
print("PREVIZ2 %d frames  %.1fs  (%.3f s/frame)  setup %.1fs"

...
for b in blocks:
    ...
    if t1 <= 0 or t0 >= 51.625: continue
    a = max(int(round(max(t0, 0.0) * FPS)), 0)
    z = min(int(round(min(t1, 51.625) * FPS)), 1238)
    if z <= a: continue
    used += 1
...
subprocess.run(["ffmpeg", "-y", "-v", "error", "-i", SRC, "-vf", vf,
                ...
                "-pix_fmt", "yuv420p", "-an", "_a6tx2/_cust_v.mp4"], check=True)
subprocess.run(["ffmpeg", "-y", "-v", "error", "-i", "_a6tx2/_cust_v.mp4",

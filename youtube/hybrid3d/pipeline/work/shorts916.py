...
CUTS = [
    ...
    # [CEO-83 축④ 커버 / replan.py P4a]  A3-16 / A4-01 두 컷을 ★제외★ 한다.
    #   이유: 대본이 그 두 컷에서 앵커(조건 카드)를 ★뗀다★.
    #     A3-16 screen_direction "회의 브리프 1장과 옆의 개인 설계 노트" -> 주연은 brief
    #     A4-01 screen_direction "결과 보관 서가로 수직 상승"        -> 주연은 res2
    #   앵커를 억지로 끼우면 교훈 200(대본이 서사의 정본) / CEO-74 위반이다.
    #   대안 검토 기록:
    #     P4b 두 컷에 앵커 주입 -> ★금지★ (대본 위반)
    #     P4c 앵커가 살아있는 다른 컷을 붙여 길이 보충 -> ★기각★.
    #         A5-01/02/04 는 카드가 ★공간 이동★ 하는 완벽한 근거(축⑤)지만
    #         293~298초로 숏폼 C 사용 구간(206~225초) ★밖★ 이고 PROPS 미작성이다.
    #         (74_SHORTS_TRILOGY_SPEC.md = 사용 구간 정본 · 교훈 216)
    #     ★P4a 두 컷 제외 -> 채택★. 커버 62% -> ★100%★ (384/384 f = ★16.00s★),
    #         숏폼 하한 15s 충족. 4컷 전부 앵커 card 정체성 1종.

# -*- coding: utf-8 -*-
"""3편 썸네일: AI 배경(흩어진 점->정렬된 설계도) + navy+gold 텍스트 합성. 1280x720 유튜브 표준.
가치 정합성: 자기과시/통제 욕망 배제, 겸손·청지기('막연한 바람->한 장의 설계도') 은유."""
...
for x in range(W):
    a=int(175*max(0,(1-x/(W*0.74))))  # 왼쪽 진하게
...
# 메인 카피 (2줄, 대형)
main_f=F(EB,118)
shadow_text((70,196),"막연한 바람을",main_f,WHITE)
# 둘째 줄 '한 장의 설계도로' gold glow 강조
glow_text((70,330),"한 장의 설계도로",main_f,GOLD_LT,glow=GOLD,gr=14,ga=170)
...
# 하단 보조 카피
sub_f=F(R,40)
shadow_text((74,516),"설계라는 두 번째 열쇠",sub_f,GREY,sa=150,sr=5,off=3)
...
bg.save("thumbnail_3.png")
print("thumbnail saved -> thumbnail_3.png", bg.size)

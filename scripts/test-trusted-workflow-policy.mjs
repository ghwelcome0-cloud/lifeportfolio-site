...
for(const [name,mutate] of[["step-delete",x=>x.replace(/      - name: Validate internal evidence trust root[\s\S]*?      - run: npm test/,"      - run: npm test")],["step-rename",x=>x.replace("Validate internal evidence trust root","Evidence")],["command-delete",x=>x.replace("          node scripts/test-ropa-evidence.mjs\n","")],["env-weaken",x=>x.replaceAll("EVIDENCE_BASE_SHA: ${{ github.event.pull_request.base.sha || '' }}","EVIDENCE_BASE_SHA: ${{ github.sha }}")]]){const candidate=mutate(requiredWorkflow);const ok=(candidate.match(/name:\s*Validate internal evidence trust root/g)||[]).length===1&&(candidate.match(/EVIDENCE_BASE_SHA:\s*\$\{\{\s*github\.event\.pull_request\.base\.sha\s*\|\|\s*''\s*\}\}/g)||[]).length===2&&["test-ropa-evidence","test-legal-authority-registry","test-internal-evidence-dlp","test-internal-evidence-contract"].every(c=>(candidate.match(new RegExp(c,"g"))||[]).length===1);assert.equal(ok,false,`required mutation must fail: ${name}`);}

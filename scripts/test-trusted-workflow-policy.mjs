...
assert.match(requiredWorkflow,/name:\s*Validate legal L0 router against exact source/);assert.match(requiredWorkflow,/CONTRACT_BASE_SHA:\s*\$\{\{ github\.event_name == 'pull_request' && github\.event\.pull_request\.base\.sha \|\| github\.sha \}\}/);assert.match(requiredWorkflow,/PR_HEAD_SHA:\s*\$\{\{ github\.event_name == 'pull_request' && github\.event\.pull_request\.head\.sha \|\| github\.sha \}\}/);assert.match(requiredWorkflow,/CONTRACT_PR_NUMBER:\s*\$\{\{ github\.event_name == 'pull_request' && github\.event\.pull_request\.number \|\| '0' \}\}/);assert.match(requiredWorkflow,/CONTRACT_EVENT_NAME:\s*\$\{\{ github\.event_name \}\}/);assert.match(requiredWorkflow,/run:\s*node scripts\/run-legal-l0-router\.mjs/);

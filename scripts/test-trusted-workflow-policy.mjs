...
assert.match(requiredWorkflow,/name:\s*Validate legal L0 current PR router/);assert.match(requiredWorkflow,/if:\s*github\.event_name == 'pull_request'/);assert.match(requiredWorkflow,/CONTRACT_BASE_SHA:\s*\$\{\{ github\.event\.pull_request\.base\.sha \}\}/);assert.match(requiredWorkflow,/PR_HEAD_SHA:\s*\$\{\{ github\.event\.pull_request\.head\.sha \}\}/);assert.match(requiredWorkflow,/CONTRACT_PR_NUMBER:\s*\$\{\{ github\.event\.pull_request\.number \}\}/);assert.match(requiredWorkflow,/run:\s*node scripts\/run-legal-l0-router\.mjs/);const legalStep=requiredWorkflow.slice(requiredWorkflow.indexOf("- name: Validate legal L0 current PR router"),requiredWorkflow.indexOf("- name: Validate internal evidence trust root"));assert.doesNotMatch(legalStep,/CONTRACT_BASE_SHA:[^\n]*(?:\|\||github\.sha)/);

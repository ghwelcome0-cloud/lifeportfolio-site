...
assert.equal(ko.program.lp_repro_hd,"계산 재현성 · 응답 코드");requireAll("program i18n KO key",ko.program.lp_repro_p,[/동일 release/,/동일 엔진/,/동일 입력/,/가명 식별자/,/유일성/,/사람의 변화/,/재검사 신뢰도/,/타당도/,/과거 release 재생성은 아직 구현되지/]);
assert.equal(en.program.lp_repro_hd,"Calculation reproducibility · response code");requireAll("program i18n EN key",en.program.lp_repro_p,[/same release, engine, and input/,/pseudonymous identifier/,/uniqueness/,/human change/,/test-retest reliability/,/validity/,/replay of older releases is not implemented/]);
...
const reportEnBranch=report.match(/var reproP=isEn\?"([^"]+)":"([^"]+)";/);assert.ok(reportEnBranch,"report IX/X exact KO/EN branch missing");requireAll("report IX/X EN branch",reportEnBranch[1],[/same release, engine, and input/,/pseudonymous identifier/,/uniqueness/,/human change/,/test-retest reliability/,/validity/,/replay of older releases is not implemented/]);requireAll("report IX/X KO branch",reportEnBranch[2],[/동일 release/,/가명 식별자/,/유일성/,/사람의 변화/,/재검사 신뢰도/,/타당도/,/과거 release 재생성은 아직 구현되지/]);

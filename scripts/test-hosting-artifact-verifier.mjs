...
let root=fixture({"index.html":"support@example.org 010-1234-5678"});
assert.equal(verifyArtifact(root,{policyPath:policy(root,approved)}).files,5);
expectDlp({"product.html":"support@example.org"},approved);
expectDlp({"index.html":"other@example.org"},approved);
...
for (const changed of [[],[{value:"support@example.org",files:["product.html"]}]]) {
  const r=fixture({"index.html":"support@example.org"}); const p=policy(r,changed);

...
try{
 for(const lang of ["ko","en"]){
  ...
  const p=await browser.newPage();await p.setRequestInterception(true);let reportDocument=true;const intercept=r=>{if(!r.isInterceptResolutionHandled()){if(r.isNavigationRequest()&&r.resourceType()==="document"&&!reportDocument)r.abort();else{if(r.isNavigationRequest()&&r.resourceType()==="document")reportDocument=false;r.continue();}}};p.on("request",intercept);await p.evaluateOnNewDocument(r=>window.__SAMPLE__=r,report);await p.goto(base+"/report.html",{waitUntil:"domcontentloaded"}).catch(()=>{});await p.waitForFunction(()=>typeof window.__buildBookHTML==="function",{timeout:15000});const html=await p.evaluate(()=>window.__buildBookHTML("screen"));p.off("request",intercept);await p.setRequestInterception(false);await p.setContent(html,{waitUntil:"domcontentloaded"});

...
try{
 for(const lang of ["ko","en"]){
  ...
  const p=await browser.newPage();await p.goto(base+"/report.html",{waitUntil:"domcontentloaded"});await p.evaluate(r=>window.__SAMPLE__=r,report);await p.waitForFunction(()=>typeof window.__buildBookHTML==="function");const html=await p.evaluate(()=>window.__buildBookHTML("screen"));await p.setContent(html,{waitUntil:"domcontentloaded"});
  ...
  for(const width of [1440,320,375,430]){await p.setViewport({width,height:1000});await p.reload({waitUntil:"domcontentloaded"}).catch(()=>{});await p.setContent(html,{waitUntil:"domcontentloaded"});const geom=await p.$$eval('[data-anchor="ch9"],[data-anchor="ch10"]',xs=>xs.map(x=>({sw:x.scrollWidth,cw:x.clientWidth,r:x.getBoundingClientRect(),kids:[...x.querySelectorAll("*")].some(k=>{const r=k.getBoundingClientRect();return r.left<x.getBoundingClientRect().left-1||r.right>x.getBoundingClientRect().right+1;})})));for(const g of geom){assert.ok(g.sw<=g.cw+1,`${lang}/${width}: horizontal overflow`);assert.equal(g.kids,false,`${lang}/${width}: clipped child`);}}
  ...
  const pp=await browser.newPage();await pp.goto(base+"/program.html",{waitUntil:"domcontentloaded"});await pp.waitForFunction(()=>typeof window.__buildProgramBookHTML==="function");const ph=await pp.evaluate(x=>window.__buildProgramBookHTML("screen",x),program);assert.match(ph,lang==="en"?/response code/:/응답 코드/);await pp.close();await p.close();
 }

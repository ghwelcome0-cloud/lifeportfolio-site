...
const approvalsFixture = [{ schema_version: "internal-evidence-approval.v1", evidence_id: "IE-APPROVAL-001", actor_id: "owner_actor", actor_role: "owner", decision: "Approved", decided_at: "2026-08-13T23:00:00Z", reviewed_head_sha: context.headSha, pull_request: 999, migration_id: "IE-MIG-001" }, { schema_version: "internal-evidence-approval.v1", evidence_id: "IE-APPROVAL-002", actor_id: "reviewer_actor", actor_role: "independent_reviewer", decision: "Approved", decided_at: "2026-08-13T23:10:00Z", reviewed_head_sha: context.headSha, pull_request: 999, migration_id: "IE-MIG-001" }];
if (!validateMigration(migration) || approvalsFixture.some((x) => !validateApproval(x)) || validateMigrationBundle({ migration, approvals: approvalsFixture, allMigrations: [migration], allApprovals: approvalsFixture, context, baseRegistryDigest: "a".repeat(64), currentRegistryDigest: "b".repeat(64), now }).length) failures.push("valid future migration bundle rejected");

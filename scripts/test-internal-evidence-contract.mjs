...
import { validateMigrationBundle, validateMigrationHistory, loadJsonDirectory } from "./internal-evidence-migration-lib.mjs";
...
if (!validateMigration(migration) || approvalsFixture.some((x) => !validateApproval(x)) || validateMigrationBundle({ migration, approvals: approvalsFixture, context, baseRegistryDigest: "a".repeat(64), currentRegistryDigest: "b".repeat(64), now, verifiedEvidenceIds: approvalsFixture.map((x) => x.evidence_id), history: validateMigrationHistory([migration], approvalsFixture) }).length) failures.push("valid future migration bundle rejected");

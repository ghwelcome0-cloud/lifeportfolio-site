...
export function routePolicyContract(changed,currentManifest,baseManifest){
  ...
  const activationMaterial=new Set(["contracts/activation.json","contracts/public-contact-policy.schema.json",...((currentManifest?.contracts?.public_contact_policy?.files)||[]),...((baseManifest?.contracts?.public_contact_policy?.files)||[])].filter(f=>!nonMaterial.has(f)));
  const contractChanged=changed.some(f=>activationMaterial.has(f));
  return contractChanged||unknownPrefix?"activation":"steady";
}

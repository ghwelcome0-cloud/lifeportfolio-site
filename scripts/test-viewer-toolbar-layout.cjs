...
try{for(const file of ['report.html','program.html']){
 ...
 const scripts=s=>Array.from(s.matchAll(/<script\b[^>]*>[\s\S]*?<\/script>/gi),m=>m[0]);assert.deepEqual(scripts(source),scripts(before),'All runtime scripts must remain byte-identical');
 ...
 await page.close();
}console.log('PASS '+count+' toolbar layouts; runtime scripts unchanged');

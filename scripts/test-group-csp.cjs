...
try{
 for(const file of ['suvey.html','b2b-join.html'])for(const suffix of ['', '.html'])for(const width of [375,1280])for(const scenario of ['success','server-denial','blocked-header','blocked-unrelated',...(file==='suvey.html'?['old-meta']:[])]){
  ...
  else if(scenario==='blocked-unrelated') {assert.equal(reached,0);if(file==='suvey.html')assert.ok(result.violations.length>0);}

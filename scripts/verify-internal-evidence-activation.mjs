...
if (event === "push" || event === "workflow_dispatch") {
  ...
  if (!baseHasRoot) {
    ...
    if (migrations.length === 1) {
      const provenance = await verifyApprovalProvenance(approvals, { repository: process.env.GITHUB_REPOSITORY, token: process.env.GH_TOKEN, expectedActors: { owner: process.env.EVIDENCE_EXPECTED_OWNER, independent_reviewer: process.env.EVIDENCE_EXPECTED_REVIEWER } });

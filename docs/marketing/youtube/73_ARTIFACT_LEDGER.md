...
## 앵커 5축 재계측 검증 이력 ([CEO-83] · 교훈 230)

### 도구 (영구 파일 · 미러 동시 커밋 `b738576`)

| 파일 | 축 | 역할 |
|---|---|---|
| `r3d/anchorlib.py` | 공용 | `script_gate` 에서 임계·산식 import (교훈 176) + `selfcheck()` |
| `r3d/visib.py` | ② | 두께비 = 바 두께 px / 같은 프레임 카드 장변 px (거리 불변) |
| `r3d/needsize.py` | ④ | 비트별 필요 최장변 · 최악 화면폭 · 커버 % |
| `r3d/altsweep.py` | ①+⑤ | 후보색 6종 대비/채도 · 크기·렌즈 스윕 |

### 자기검사 — §7 표 12개 값 재현 (12/12 OK · 0 fails)
...
**확정 산식** — 색: sRGB linearize → WCAG 상대휘도 / 채도: HSV `(max-min)/max` /
대비: `(hi+0.05)/(lo+0.05)` / 투영: `px = 2*max(sc) * lens / (dep*SENSOR_MM) * RES_W` /
필요: `need_edge = FRAC_MIN * dep * SENSOR_MM / lens`

### 재계측 결과

```
축①  1.90:1  ->  8.11:1  ✅   (현행 DOC_ANCHOR_C · 판정식 AND -> OR 교정)
축②  ◐       ->  x1.558  ✅   (두께비 0.0542 -> 0.0844 · 거리효과 x1.307 제거 후 x1.697)
축③  ❌ 46%   ->  비 1.24 ◐   (육안 미확인 — 유일한 잔여)
축④  ❌ 62%   ->  67% · 카드컷 하한 통과 2/4 ◐
축⑤  ❌       ->  크기 x1.30 (A4 x1.14) 으로 4/4 ◐
```

### 축④ 발견 상세 — 게이트가 놓친 차원

```
비트    lens  최원거리m  필요최장변m  A4배수  실제최장변m  최악화면폭  판정
A3-13   40.1    2.523      0.3171    1.07     0.2600     0.1148   미달
A3-14   50.0    1.761      0.1775    0.60     0.2600     0.2051   OK
A3-15   54.0    1.335      0.1246    0.42     0.2600     0.2921   OK
A3-16   82.5    4.193      0.2562    0.86       -          -      카드없음
A3-17   40.0    2.336      0.2943    0.99     0.2600     0.1237   미달
A4-01   45.0    3.260      0.3651    1.23       -          -      카드없음
```

원인 — **G6 는 「컷의 최대 주연」만** 본다(A3-13 의 최대 주연은 `cmptab`),
**G9 는 「비」와 「커버」만** 본다(절대 크기 미측정). 미보유 게이트는
「앵커 절대 크기 하한」이고 `needsize.py` 가 대신한다.

**미적용 결정** — A3-13/A3-17 은 `SHORTS_C_LOCK` = CEO 승인 납품본 구성 컷.
교훈 131 + [CEO-85] ⇒ 계산만 하고 선택지를 남긴다 (숏폼 D 안건).

...
| 문서 | 무엇 | 누가 읽나 | 갱신 |
|---|---|---|---|
| [`00_CEO_DIRECTIVES.md`](./00_CEO_DIRECTIVES.md) | **대표님 지시 원문 전량 (CEO-1~85)** + 항구 지침 | 전원 | 추가만 |
...
| [`71_HYBRID_3D_PRODUCTION_RULES.md`](./71_HYBRID_3D_PRODUCTION_RULES.md) | **따라 하면 같은 품질이 나오는 실행 규칙** — 교훈 1~221 | 제작 실행자 | 추가만 |
| [`77_PRODUCTION_SOP.md`](./77_PRODUCTION_SOP.md) | **★상시 생산 표준 (SOP)★** — 이 문서 하나로 영상 한 편을 끝까지 만든다. 파이프라인 7단계 · 게이트 12종 · 실행 명령 전량 · 납품 우선 원칙 (CEO-85 ⑤) | **제작 실행자 · 최우선** | 추가만 |
